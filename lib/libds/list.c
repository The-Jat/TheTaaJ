...
List_t*
ListCreate(KeyType_t KeyType, Flags_t Attributes)
{
...
OsStatus_t ListRemoveByKey(List_t *List, DataKey_t Key)
{
	...
	if (Node != NULL) {
		if (ListRemoveByNode(List, Node) != Success
			|| ListDestroyNode(List, Node) != Success) {
			return Error;
		}
		else {
			return Success;
